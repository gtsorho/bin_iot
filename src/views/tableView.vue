  ...
  <div class="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
    <div class="rounded-t mb-0 px-4 py-3 border-0">
      <div class="flex flex-wrap items-center">
        <div class="relative w-full px-4 max-w-full flex-grow flex-1">
          <i class="fas mr-4 fa-arrow-left ring-black ring  rounded-full p-1 border-black inline text-black" @click="showRecord = false" v-if="showRecord"></i>
    ...
    <div class="block w-full overflow-x-auto" v-if="!showRecord">
      <table class="items-center bg-transparent w-full border-collapse ">
        ...
        <tbody>
          <tr class="hover:bg-gray-200" @click="getRecord(client.id)" v-for="(client, i) in clients" :key="i" >
            ...
            <td class="border-t-0 px-6 align-center border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              <i class="fas mr-4" :class="client.level >= 80 ? 'fa-arrow-down text-emerald-500': 'fa-arrow-up text-red-500'"></i>
              {{ 100 - client.level }}
            </td>
            <td class="border-t-0 px-6 align-middle  border-l-0 border-r-0 text-xs whitespace-nowrap p-4"><p class="px-5 py-1 text-center uppercase rounded-md w-32 "  :class="getThreshold(100 - client.level)" > {{ client.remark }}</p></td>
      ...
      <table class="items-center bg-transparent w-full border-collapse ">
        <thead>
          <tr class="hover:bg-gray-200">
            ...
            <th class="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-center">Is Verified</th>
          </tr>
        </thead>
        ...
        <tbody>
          <tr class="hover:bg-gray-200" v-for="(log, i) in record.records" :key="i">
            ...
            <td class="border-t-0 px-6 align-middle text-center border-l-0 border-r-0 text-xs whitespace-nowrap p-4 ">{{ 100 - log.level }}</td>
...
export default {
  ...
  methods: {
    ...
    getRecord(id) {
      ...
      axios.get('http://bin.greatergraceag.com/api/records/'+ id, { headers:{'Authorization': `Bearer ${this.token}`}})
        .then((response) => {
