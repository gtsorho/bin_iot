    ...
    <div class="relative flex h-16 items-center justify-between">
      ...
      <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
        ...
        <div class="relative ml-3">
          ...
          <div v-if="showDropdown" class="absolute leaflet-control right-0 z-10 mt-0 w-64 origin-top-right rounded-md  bg-white py-1 px-2 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" style="z-index:1000" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
            <a href="#" @click="showDropdown = !showDropdown" class="block px-4 py-2 text-sm hover:bg-gray-200 my-1 border border-1 border-red-100 text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-0" v-for="(notification, i) in notifications" :key="i">
              {{`${notification.name} at ${notification.location} is ${notification.remark}`}}
              </a>
          </div>
        </div>
